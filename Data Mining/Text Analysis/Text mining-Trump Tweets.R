...
p <- ggplot(subset(wf, freq>50), aes(reorder(word, -freq), y = freq)) +geom_bar(stat = "identity") + theme(axis.text.x=element_text(angle=45, hjust=1))
p  
...
findAssocs(dtm, c("nation", "people"), corlimit=0.85) #Nation and all words appear together in one file for for 95%

findAssocs(dtm, c("people", "going"), corlimit=0.85)
